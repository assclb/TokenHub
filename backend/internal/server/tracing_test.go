...
func TestTracingRecordsFailoverPerCandidate(t *testing.T) {
	...
	app.adapters["partial_usage_mock"] = partialUsageFailingAdapter{}
...
func TestTracingCountsExportOutcomes(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			if got := counterValue(t, app, "tokenhub_gateway_trace_completions_total", traceCompletionOutcomeQueued); got < 1 {
				t.Fatalf("expected a queued completion, got %v", got)
			}
