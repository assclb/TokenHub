...
| `tokenhub_gateway_trace_completions_total` | counter | Finished calls by what trace export did with them: `queued` or `dropped`. Only present when tracing is on. |
...
The attribute mapping targets Langfuse v4, which is what Langfuse Cloud runs and what self-hosted deployments should be on. Self-hosting Langfuse v4 requires ClickHouse 25.12 or newer alongside PostgreSQL, Redis and object storage. TokenHub does not bundle Langfuse into its own Compose files: that is a second stateful stack with its own upgrade cycle, and coupling the two would make a Langfuse migration a gateway outage.
...
Trace and span IDs are both derived from the request ID. The `x-request-id` header returned to a client therefore leads straight to its trace without a lookup table, and because OTLP delivery is at-least-once — an acknowledgement lost in transit makes the exporter resend — a retried batch rewrites the same observations instead of adding a second copy of the trace. Playground traffic is exported with a `playground` tag so it can be excluded from cost analysis, and requests refused before routing are exported too, since a quota or admission failure is usually what you are trying to explain.
